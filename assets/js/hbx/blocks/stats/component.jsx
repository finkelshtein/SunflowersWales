...
export const StatsBlock = ({content, design, _id, icon_svgs}) => {
  ...
  // Sunflowers Wales: for the `compact` layout, the connecting sentence
  // (content.text) renders as a bold heading *inside* the white card itself
  // rather than floating above it in a separate header — reads as "this
  // card is about ...' instead of a disconnected caption. `title`, and
  // `text` on the other layouts, keep the original shared-header treatment.
  const textInCard = layout === "compact" && !!text;

  return (
    <div class="py-8 sm:py-12 lg:py-16" ref={containerRef}>
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        {(title || (text && !textInCard)) && (
          <div class="text-center mb-12 lg:mb-16">
            ...
            {text && !textInCard && (
              <p class="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto" dangerouslySetInnerHTML={{__html: renderText(text)}} />
            )}
          </div>
        )}
        ...
        {layout === "compact" && (
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            {textInCard && (
              <p
                class="text-center font-bold text-base sm:text-lg text-gray-900 dark:text-white px-6 pt-5 pb-4 border-b border-gray-200 dark:border-gray-700"
